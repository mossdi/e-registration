...
<div class="site-signup">
    <div class="row">
        ...
        <?php if ((Yii::$app->user->can('admin') || Yii::$app->user->can('receptionist')) && $model->scenario == UserForm::SCENARIO_CREATE): ?>
            <div class="col-xs-12">
                <?php try {
                    echo AutoComplete::widget([
                        ...
                        'clientOptions' => [
                            ...
                            formLoad(\'/user/signup-form?scenario=' . UserForm::SCENARIO_CONFERENCE . '\', ui.item.label, ui.item.value);
                        }'),
                        ],
        ...
        <div class="col-xs-12">
            <div class="row">
                ...
                <?= $form->field($model, 'last_name', ['options' => ['class' => 'col-xs-4']])
                    ->textInput(['placeholder' => 'Фамилия', 'readonly' => $model->scenario == UserForm::SCENARIO_CONFERENCE ? true : false]) ?>

                <?= $form->field($model, 'first_name', ['options' => ['class' => 'col-xs-4']])
                    ->textInput(['placeholder' => 'Имя', 'readonly' => $model->scenario == UserForm::SCENARIO_CONFERENCE ? true : false]) ?>
                ...
                <?= $form->field($model, 'organization', ['options' => ['class' => 'col-xs-6']])
                    ->textInput(['placeholder' => 'Оргнанизация', 'readonly' => $model->scenario == UserForm::SCENARIO_CONFERENCE ? true : false]) ?>

                <?= $form->field($model, 'post', ['options' => ['class' => 'col-xs-6']])
                    ->textInput(['placeholder' => 'Должность', 'readonly' => $model->scenario == UserForm::SCENARIO_CONFERENCE ? true : false]) ?>

                <?= $form->field($model, 'phone', ['options' => ['class' => 'col-xs-6']])
                    ->widget(MaskedInput::className(), ['mask' => '+7 (999) 999-99-99'])
                    ->textInput(['placeholder' => 'Телефон', 'readonly' => $model->scenario == UserForm::SCENARIO_CONFERENCE ? true : false]) ?>

                <?= $form->field($model, 'email', ['options' => ['class' => 'col-xs-6']])
                    ->textInput(['placeholder' => 'Эл.почта', 'readonly' => $model->scenario == UserForm::SCENARIO_CONFERENCE ? true : false]) ?>
                ...
                <?php if ((Yii::$app->user->can('admin') || Yii::$app->user->can('receptionist')) && $model->scenario != UserForm::SCENARIO_UPDATE):
                ...
                <?php if (Yii::$app->user->can('admin') && $model->scenario == UserForm::SCENARIO_CREATE):
            ...
            <div class="form-group">
                ...
                <?php if ($model->scenario == UserForm::SCENARIO_CONFERENCE):
